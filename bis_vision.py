...
class PatternMatcher:
    ...
    def __init__(self, template_root: str, state: GameState = None):
        ...
        self._generate_patterns_json(template_root)  # patterns.json 자동 생성
    ...
    def decode_ahk_pattern(self, ahk_string: str) -> np.ndarray:
        """
        AHK FindText 패턴 문자열을 numpy 2차원 배열로 변환
        형식: |<숫자>*파라미터$너비.데이터
        예: |<0>*117$18.zzzzzzzVzzVzy0Dy0Dy0DkS3kS3kS3kS3kS3kS3kS3kS3kS3kS3y0Dy0DzVzzVzzzzzzzzzzU
        """
        try:
            # 파싱: |<0>*117$18.zzzzz...
            # 1. 숫자 식별자 제거
            if not ahk_string.startswith("|<"):
                raise ValueError("Invalid AHK pattern format")
            
            # 2. 너비 추출
            dollar_idx = ahk_string.find('$')
            if dollar_idx == -1:
                raise ValueError("No width separator found")
            
            width_part = ahk_string[dollar_idx + 1:]
            dot_idx = width_part.find('.')
            if dot_idx == -1:
                raise ValueError("No data separator found")
            
            width = int(width_part[:dot_idx])
            data_str = width_part[dot_idx + 1:]
            
            # 3. 데이터 디코딩 (Base64 변형 → 0/1)
            # AHK FindText는 64진법 변형을 사용
            # 각 문자를 6비트로 변환
            bitmap = []
            row = []
            ...
            for char in data_str:
                ...
                if char == 'z':
                    ...
                    # ASCII 코드 기반 변환
                    val = ord(char) - 48  # '0' = 48
                    if val < 0:
                        val = ord(char) - 65 + 10  # 'A' = 65
                    bits = format(val, '06b')
            ...
            print(f"[AHK] 패턴 디코딩 실패: {e}")
            return np.zeros((10, 10), dtype=np.uint8)

    def _generate_patterns_json(self, root: str):
        """AHK FindText 패턴을 기반으로 patterns.json 자동 생성"""
        patterns_file = os.path.join(root, "patterns.json")
        ...
        try:
            ...
            for digit, ahk_pattern in self.AHK_PATTERNS.items():
                bitmap = self.decode_ahk_pattern(ahk_pattern)
                if bitmap is not None and bitmap.size > 0:
                    patterns[digit] = {
                        "width": bitmap.shape[1],
                        "height": bitmap.shape[0],
                        "bitmap": bitmap.tolist(),  # numpy array → list
                        "ahk_pattern": ahk_pattern
                    }
            
            with open(patterns_file, 'w', encoding='utf-8') as f:
                json.dump(patterns, f, indent=2, ensure_ascii=False)
            
            print(f"[Pattern] patterns.json 자동 생성 완료: {len(patterns)}개 숫자 패턴")
        except Exception as e:
            print(f"[Pattern] patterns.json 생성 실패: {e}")

    def _load_digit_templates(self, root: str):
        """?자 ?플릿을 HSV ?상 기반?로 로드 (초록???금??"""
        ...
        for i in range(10):
            ...
            body = combined_mask[y:y + h, x:x + w]

            # ?규모양 추출
            resized = cv2.resize(body, (tw, th), interpolation=cv2.INTER_AREA)
            _, binary = cv2.threshold(resized, 1, 1, cv2.THRESH_BINARY)

            self.digit_templates[i] = {
                "data": binary.astype(np.uint8),
                "pixels": int(np.sum(binary)),
                "ratio": w / max(h, 1),
            }

        print(f"[PM] Digit templates loaded: {list(self.digit_templates.keys())}")
    ...
    def _load_all(self, root: str):
        ...
        for folder in self.FOLDERS:
            ...
            if not os.path.isdir(folder_path):
                print(f"[PM] {folder} 폴더 없음 - 건너?")
                continue
            loaded_in_folder = 0
            for fn in sorted(os.listdir(folder_path)):
                if not fn.lower().endswith((".png", ".jpg", ".bmp")):
                    continue
                img = cv2.imread(os.path.join(folder_path, fn), cv2.IMREAD_COLOR)
                if img is not None:
            ...
            if loaded_in_folder > 0:
                print(f"[PM] {folder} folder: {loaded_in_folder} templates loaded.")

        counts = {f: len(v) for f, v in self.templates.items()}
        print(f"[PM] PatternMatcher Load Complete: Total {loaded_total} templates. {counts}")
    ...
    def findtext_scan(self, play_area_rgb: np.ndarray, folders: list[str] = None,
        ...
        _dbg_dir = _dbg_os.path.dirname(_dbg_os.path.abspath(__file__))
        cv2.imwrite(_dbg_os.path.join(_dbg_dir, "debug_preprocess_gray.png"), gray)
        cv2.imwrite(_dbg_os.path.join(_dbg_dir, "debug_preprocess_enhanced.png"), enhanced)
        cv2.imwrite(_dbg_os.path.join(_dbg_dir, "debug_preprocess_binary.png"), src_bin)
        ...
        for folder in folders:
            ...
            for cat_name, variants in self.binary_templates[folder].items():
                for v in variants:
                    ...
                    # 디버그: hobak 템플릿 이진화 이미지 저장
                    if cat_name == "hobak" and not hasattr(self, '_dbg_hobak_saved'):
                        cv2.imwrite(_dbg_os.path.join(_dbg_dir, "debug_template_hobak_bin.png"), tmpl)
                        self._dbg_hobak_saved = True
    ...
    def recognize_entity_bitwise(self, crop_rgb: np.ndarray, folder: str,
        ...
        crop_rgb를 이진화(127 임계값) 후 템플릿과 1:1 XOR 비교.
        match_score >= threshold(0.80)이면 매칭 성공.
        실패 시에만 (w, h) 로그 출력, 성공 시 간단히 "감지 성공".
        """
        if crop_rgb is None or crop_rgb.size == 0:
            return "", 0.0
        if folder not in self.binary_templates or not self.binary_templates[folder]:
        ...
        for cat_name, variants in self.binary_templates[folder].items():
            for v in variants:
                ...
                # 크기 필터 (20% 허용)
                if not (t_w * 0.8 <= crop_w <= t_w * 1.2 and t_h * 0.8 <= crop_h <= t_h * 1.2):
                    continue

                # 템플릿을 crop 크기에 맞게 리사이즈 (1:1 비교를 위해)
                tmpl_bin = v["tmpl"]
                ...
                if score > best_score:
                    best_score = score
                    best_name = cat_name
                    best_log_info = f"덩어리 {crop_w}x{crop_h} vs 템플릿 {t_w}x{t_h} ({v['raw_name']}) | 점수: {score:.2f}"

        if best_score >= threshold:
            print(f"[Entity] 감지 성공: {best_name} (점수: {best_score:.2f})")
            return best_name, best_score

        # 실패 시에만 로그
        if best_log_info:
            self.state.last_vision_error = f"{best_log_info}"

        return "", 0.0

    def recognize_digit_bitwise(self, crop_rgb: np.ndarray) -> tuple[str, float]:
        """
        AHK FindText 패턴을 사용한 숫자 인식 (비트맵 XOR 매칭)
        crop_rgb를 이진화 후 AHK 패턴과 슬라이딩 윈도우 XOR 비교
        """
        if crop_rgb is None or crop_rgb.size == 0:
            return "", 0.0
        ...
        if len(crop_rgb.shape) == 3 and crop_rgb.shape[2] == 3:
            ...
            gray = cv2.cvtColor(crop_rgb, cv2.COLOR_RGB2GRAY)
        _, crop_bin = cv2.threshold(gray, 127, 1, cv2.THRESH_BINARY)  # 0/1로 이진화
        crop_h, crop_w = crop_bin.shape[:2]
        ...
        for digit, ahk_pattern in self.AHK_PATTERNS.items():
            # AHK 패턴 디코딩
            template = self.decode_ahk_pattern(ahk_pattern)
            ...
            # 슬라이딩 윈도우 매칭
            max_score = 0.0
            for y in range(crop_h - tmpl_h + 1):
                for x in range(crop_w - tmpl_w + 1):
                    # ROI 추출
                    roi = crop_bin[y:y+tmpl_h, x:x+tmpl_w]
                    if roi.shape != template.shape:
                        continue

                    # XOR 비교
                    xor_diff = np.bitwise_xor(roi, template)
                    diff_count = np.count_nonzero(xor_diff)
                    total_pixels = tmpl_w * tmpl_h
                    score = 1.0 - (diff_count / float(max(1, total_pixels)))
...
class SentinelThread(threading.Thread):
    ...
    def run(self):
        print("[Sentinel] 몬스터/아이템 감지 스레드 가동... (FindText XOR 모드, Target 15 FPS)")
        while self.state.running:
            now = time.time()
            # 15 FPS 제한 (약 66ms 주기)
            if now - self._last_run_time < 0.066:
                time.sleep(0.01)
                continue
...
class MonitorSvc(threading.Thread):
    ...
    def __init__(self, state: GameState, config_file: str,
        ...
        self.ocr_threshold    = 0.65  # 기본 ?식 ?계?
        self.ocr_preview_img  = None  # ?시인식 ?역 미리보기 ?이??
            ...
            for cnt in sorted_cnts:
                ...
                if w >= 800:
                    ...
                    if 1.5 < ratio < 1.9:
                        ...
                        print(f"[Vision] 게임 ?면 감?: ({x}, {y}) {w}x{h} [?라?언??{client_h}p 기? 배율: {scale:.3f}x]")
                        found = True
                        break
    ...
    def load_maps(self):
        if os.path.exists(self.maps_file):
            ...
            print(f"[Map] maps.json 로드 ?료 ({len(self.state.maps_db)}??.")
        else:
            print("[Warn] maps.json 없음 ??Grid 기능 비활??")
    ...
    # ----------------------------------------------------------
    def run(self):
        print("[Vision] V5 데이터 분석 스레드 시작... [OBS 가??카메??모드]")
        
        # 카메???결 ?도
        self.cap, self.camera_index = get_camera()
        if self.cap is None:
            print("[Error] 카메???결 실패, ?레??종료")
            return
        ...
        while self.state.running:
            ...
            if self.cap is None or not self.cap.isOpened():
                if self.retry_count < self.max_retries:
                    print(f"[Camera] 카메???결 ??, ?연??도 ({self.retry_count + 1}/{self.max_retries})")
                    self.cap, self.camera_index = get_camera()
                    self.retry_count += 1
                    time.sleep(2)
                    continue
                else:
                    print("[Error] 최? ?연??도 초과, ?레??종료")
                    return
            else:
                self.retry_count = 0  # 연결 성공 ??retry 카운??리셋
            ...
            # 진단?? ?능???해 비활?화 ?는 매우 ?물??행 (300?레?마??
            if self.frame_count % 300 == 10:
                fh, fw = frame.shape[:2]
                debug_map = frame.copy()
                for name, reg in self.regions.items():
                    sx = int(reg.sx * scx + ox)
                    sy = int(reg.sy * scy + oy)
                    dx = int(reg.dx * scx + ox)
                    dy = int(reg.dy * scy + oy)
                    cv2.rectangle(debug_map, (sx, sy), (dx, dy), (0, 0, 255), 2)
                    cv2.putText(debug_map, name, (sx, sy - 5), 
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                
                cv2.imwrite("debug_visual_map.png", debug_map)
                # print("[Debug] 배율 보정???각??매핑 ??지 ??됨: debug_visual_map.png")
                
                # 기존 개별 ?롭 ??지?????
                debug_dir = os.path.join(os.path.dirname(self.config_file), "ocr_debug")
                if not os.path.exists(debug_dir): os.makedirs(debug_dir)
                for name, reg in self.regions.items():
                    if name in self.NUMERIC_FIELDS or name == "stat_info":
                        # 메인 OCR 루프? ?일 ?딩?로 ???(비교 검증용)
                        sx, sy = max(0, int(reg.sx * scx + ox - 5)), max(0, int(reg.sy * scy + oy - 5))
                        dx, dy = min(fw, int(reg.dx * scx + ox + 5)), min(fh, int(reg.dy * scy + oy + 5))
                        crop_img = frame[sy:dy, sx:dx]
                        if crop_img.size > 0:
                            cv2.imwrite(os.path.join(debug_dir, f"check_{name}.png"), crop_img)

            # BGR -> RGB 변환
            img_np = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            
            # shared_frame 메모리에 저장 (성능 최적화 - 복사 없이 참조)
            self.state.shared_frame = img_np
            
            # ocr_preview_img는 별도 복사 (블랙박스용)
            self.state.ocr_preview_img = img_np.copy()
            ...
            # OCR 프리뷰 파일 자동 덤프 (1초 주기)
            now_dump = time.time()
            if now_dump - self._last_preview_dump_time >= 1.0 and self.state.ocr_preview_img is not None:
                dump_path = os.path.join(os.path.dirname(self.config_file), "debug_ocr_preview.png")
                cv2.imwrite(
                    dump_path,
                    cv2.cvtColor(self.state.ocr_preview_img, cv2.COLOR_RGB2BGR)
                )
                self._last_preview_dump_time = now_dump
            ...
            for name, reg in self.regions.items():
                # ?? ?능 최적?? Tiered Analysis ????????????????
                high_priority = ["money", "hp_trig", "mp_trig", "hp", "mp", "target_info", "items_scan", "play_area"]
                if name not in high_priority and (self.frame_count % 5 != 0):
                    continue
                ...
                if name in self.NUMERIC_FIELDS:
                    result = self.matcher.recognize(crop, "digits", field_name=name)
                    if result:
                        try:
                            val = int(result)
                            if self.last_values.get(name) != val:
                                res_updates[name] = val
                                self.last_values[name] = val
                                # OCR로 읽은 월드 좌표(x,y)를 my_world_pos에 업데이트
                                if name == "x":
                                    current_y = self.state.my_world_pos[1]
                                    self.state.my_world_pos = (val, current_y)
                                elif name == "y":
                                    current_x = self.state.my_world_pos[0]
                ...
                if name.lower() == "play_area":
                    ...
                    if grid_pos:
                        ...
                        if self.frame_count % 10 == 0:
                            print(f"   [GPS] >> Me: {grid_pos} | Anchor:({feet_x},{feet_y})")
                    now_log = time.time()
            ...
            if res_updates:
                ...
                # GUI 업데이트 큐에 최신 상태 전달
                if hasattr(self.state, "gui_update_queue"):
                    try:
                        self.state.gui_update_queue.put_nowait(res_updates)
                    except Exception:
                        # maxsize=1 큐가 꽉 찬 경우 이전 값 제거 후 최신 값으로 교체
                        try:
